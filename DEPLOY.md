...
- A **DNS A record** for your subdomain (e.g. `obsidianquartz.example.com`) pointing at the VPS public IPv4 (add AAAA too if the VPS has IPv6). Traefik's HTTP-01 challenge needs this resolving first.
...
git clone https://github.com/lenacodes848/app-obsidianquartz.git obsidianquartz
cd obsidianquartz
cp .env.example .env
nano .env          # set DOMAIN and BASE_URL to your real subdomain, e.g. obsidianquartz.example.com
```
...
You also need to set `OBSIDIANQUARTZ_AUTH_HTPASSWD`, `OBSIDIANQUARTZ_SECURITY_QUESTIONS`, and
`OBSIDIANQUARTZ_SESSION_SECRET` in `.env` before launching — see
...
docker compose logs -f obsidianquartz # confirm nginx started
...
> **Deployed before Git LFS was added?** Run this once, then `git pull` and
> rebuild as usual — after that it's transparent on every future pull:
> ```bash
> sudo apt-get install -y git-lfs
> git lfs install
> ```
...
`obsidianquartz-auth-service` container (`auth-service/`). Traefik checks every request against
...
Copy the full `familyuser:$2y$12$....` output into `.env` as `OBSIDIANQUARTZ_AUTH_HTPASSWD`
...
**2. Security questions** — edit `OBSIDIANQUARTZ_SECURITY_QUESTIONS` in `.env` directly, as
plain text (this file is already gitignored and lives only on the VPS — same trust
boundary as the password hash above):

```bash
OBSIDIANQUARTZ_SECURITY_QUESTIONS=[{"q":"What is Linna's favorite fruit?","a":"mango"},{"q":"How old is our cat?","a":"7"}]
```
...
Paste the output into `.env` as `OBSIDIANQUARTZ_SESSION_SECRET`. This signs session cookies;
...
```bash
# No session — should redirect (302) to /_obsidianquartz-auth/login
...
To confirm logout works: visit `https://<your-domain>/_obsidianquartz-auth/logout` — this
...
1. Regenerate the password hash with the `htpasswd` command above (if you're
   rotating the password too), and/or edit `OBSIDIANQUARTZ_SECURITY_QUESTIONS` with new
   questions/answers.
2. **Also regenerate `OBSIDIANQUARTZ_SESSION_SECRET`** (`openssl rand -hex 32`) at the same
...
- **502 Bad Gateway:** `loadbalancer.server.port` doesn't match the real listen port (8080 for the `obsidianquartz` site, 8081 for `obsidianquartz-auth-service`).
...
the allowed range could still reach `/_obsidianquartz-auth/login` and attempt to log in:

```yaml
      - "traefik.http.middlewares.obsidianquartz-ip.ipallowlist.sourcerange=203.0.113.0/24,198.51.100.7/32"
      - "traefik.http.routers.obsidianquartz.middlewares=obsidianquartz-ip,obsidianquartz-sec,obsidianquartz-forwardauth"
      - "traefik.http.routers.obsidianquartz-auth-pages.middlewares=obsidianquartz-ip,obsidianquartz-sec,obsidianquartz-ratelimit-login"
```
...
below. These are independent of the public deployment's `OBSIDIANQUARTZ_*` variables — use a
...
instead of shipping a broken image. Deployed before Git LFS was added? Run
`sudo apt-get install -y git-lfs && git lfs install` once, then `git pull`
...
- `quartz/quartz/plugins/emitters/assets.ts` (build-time image compression — resizes and re-encodes `.png`/`.jpg`/`.jpeg` to WebP via `sharp` instead of a plain copy, and fails the build loudly on an unresolved Git LFS pointer instead of shipping a broken image; see [IMAGE-OPTIMIZATION-PLAN.md](./IMAGE-OPTIMIZATION-PLAN.md))
